...
class LoadingsController extends GetxController {
  ...
  Future<void> _fetchAdsPage(int pageKey) async {
    try {
      final result = await CustomDio(
        enableLog: true,
      ).get('super-admin/loadings?page=$pageKey');
  ...
  void showDeleteConfirmDialog(LoadingModel profile) {
    Get.defaultDialog(
      title: 'Confirm!',
      middleText: 'Are you sure you want to delete this Loading?',
      textCancel: 'No',
      textConfirm: 'Yes',
      confirmTextColor: Colors.white,
      onConfirm: () => deleteLoading(profile),
    );
  }

  void deleteLoading(LoadingModel profile) async {
    try {
      Get.back();

      await CustomDio().delete('super-admin/loadings/${profile.id}');
      CustomAlert.snackBar(msg: 'Success Loading Deleted.');
