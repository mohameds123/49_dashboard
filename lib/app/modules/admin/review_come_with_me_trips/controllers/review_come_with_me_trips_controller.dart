...
class ReviewComeWithMeTripsController extends GetxController {
  ...
  Future<void> _fetchRidesPage(int pageKey) async {
    try {
      final result = await CustomDio(enableLog: true)
          .get('admin/get-review-come-with-me-trips?page=$pageKey');
  ...
  Future<void> deleteRide(ComeWithMeTripModel comeWithMeTrip) async {
    try {
      Get.back();
      await CustomDio()
          .delete('admin/delete-come-with-me-trip/${comeWithMeTrip.id}');
      Get.defaultDialog(
        title: '',
  ...
  void showApproveRideConfirm(ComeWithMeTripModel comeWithMeTrip) {
    Get.defaultDialog(
      title: 'Confirm!'.tr,
      titleStyle: const TextStyle(color: Colors.red),
      ...
      textCancel: 'No'.tr,
      confirmTextColor: Colors.white,
      onConfirm: () => approveRide(comeWithMeTrip),
    );
  }

  Future<void> approveRide(ComeWithMeTripModel comeWithMeTrip) async {
    try {
      Get.back();
      await CustomDio()
          .get('admin/approve-come-with-me-trip/${comeWithMeTrip.id}');
      Get.defaultDialog(
        title: '',
