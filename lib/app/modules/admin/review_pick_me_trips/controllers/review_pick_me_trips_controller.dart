...
class ReviewPickMeTripsController extends GetxController {
  late final ridesPagingController =
      PagingController<int, PickMeTripModel>(firstPageKey: 1)
  ...
  Future<void> _fetchRidesPage(int pageKey) async {
    try {
      final result = await CustomDio(enableLog: true)
          .get('admin/get-review-pick-me-trips?page=$pageKey');

      final newItemsResult =
          ((result.data as Map<String, dynamic>)['data'] as List)
              .map((e) => PickMeTripModel.fromMap(e as Map<String, dynamic>))
              .toList();
  ...
  void showDeleteRideConfirm(PickMeTripModel pickMeTrip) {
    Get.defaultDialog(
      title: 'Confirm!'.tr,
      ...
      onConfirm: () => deleteRide(pickMeTrip),
    );
  }

  Future<void> deleteRide(PickMeTripModel pickMeTrip) async {
    try {
      Get.back();
      await CustomDio().delete('admin/delete-pick-me-trip/${pickMeTrip.id}');
      Get.defaultDialog(
        title: '',
        middleText: 'Ride Deleted Successfully'.tr,
        textCancel: 'Ok'.tr,
      );
      ridesPagingController.removeItem(pickMeTrip);
      Get.back();
    } catch (e) {
  ...
  void showApproveRideConfirm(PickMeTripModel pickMeTrip) {
    Get.defaultDialog(
      title: 'Confirm!'.tr,
      titleStyle: const TextStyle(color: Colors.red),
      middleText: 'Are you sure you want to approve this Ride?'.tr,
      textConfirm: 'Yes'.tr,
      textCancel: 'No'.tr,
      confirmTextColor: Colors.white,
      onConfirm: () => approveRide(pickMeTrip),
    );
  }

  Future<void> approveRide(PickMeTripModel pickMeTrip) async {
    try {
      Get.back();
      await CustomDio().get('admin/approve-pick-me-trip/${pickMeTrip.id}');
      Get.defaultDialog(
        title: '',
        middleText: 'Ride Approved Successfully'.tr,
        textCancel: 'Ok'.tr,
      );
      ridesPagingController.removeItem(pickMeTrip);
      Get.back();
