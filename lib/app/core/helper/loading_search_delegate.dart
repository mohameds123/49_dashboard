...
class LoadingSearchDelegate extends SearchDelegate {
  final Function(LoadingModel) onCardClick;
  final Function(LoadingModel) onBlockClick;
  final Function(LoadingModel) onDeleteClick;

  List<LoadingModel> results = [];

  LoadingSearchDelegate({
        ...
        builder: (_, snapshot) => results.isNotEmpty
            ? ListView.builder(
                itemBuilder: (context, index) {
                  return _buildLoadingCard(results[index]);
                },
    ...
    if (query.length > 2) {
      try {
        final result = await CustomDio().post(
          'super-admin/search-loadings',
          body: {'data': query},
        );
        results = (result.data['data'] as List)
            .map((e) => LoadingModel.fromMap(e))
            .toList();
      } catch (e) {
                ...
                children: [
                  Expanded(
                    child: MaterialButton(
                      onPressed: () => onDeleteClick(loading),
                  ...
                  Expanded(
                    child: MaterialButton(
                      onPressed: () => onBlockClick(loading),
                      child: 'Block'.text,
                      textColor: Colors.white,
